...
export default function Profile() {
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <p className='text-green-700 text-lg nt-5'>{updateSuccess ? 'User is updated successfully!' : ''}</p>

      <button onClick={handleShowListing} className='text-green-700 w-full '>Show Listing</button>

      <p className='text-red-700 nt-5'>{showListingsError ? 'Error while showing listings ' : ''}</p>

      {
        ...
        <div className='flex flex-col gap-4'>
          ...
          <div className='border rounded-lg p-3 flex justify-between item-center gap-4' key={listing._id}>
            <Link to={`/listing/${listing._id}`} >
              <img className='h-16 w-16 object-contain' src={listing.imageUrls[0]} alt="listing cover" />
            </Link>
            <Link className='text-slate-700 font-semibold hover:underline tuncate flex-1' to={`/listing/${listing._id}`} >
              <p >{listing.name}</p>
            </Link>
